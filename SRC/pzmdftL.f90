...
!      DOUBLE PRECISION, PARAMETER :: PI = 3.141592653589793238462643383279502884197169399375E+0
      DOUBLE PRECISION   PI 
!     ..
!     .. Local Scalars ..
      ...
      INTEGER              NPCOL, NPROW, NROLL, NQ, MB, M, PowK, &
                           ...
                           IWA,IWEND, IWTEMP,IWU, IWVT, IWVT1, J2, RK, &
                           LWORK, IPSV, IPWR
...
!     ..
!     .. External Functions ..
      INTEGER              ICEIL, ILCM, NUMROC, NPREROC
      EXTERNAL             ICEIL, ILCM, NUMROC, NPREROC
!     ..
!     .. Common blocks ..
...
!      IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
!         WRITE(*,*) 'A=', REAL( WORK(1:MP*MP))
!      ENDIF 
!
!     Compute the SVD of ZWORK and construct its low-rank approximation
!     For simplicity, we assume that N is dividable by NB, and N= NB*NPROW=NB*NPCOL, 
!     in Version-V0.  
! 	  
      LWORK = 3*MP
      time1 = MPI_Wtime()
      CALL ZGESVD('A','S',MP,MP,WORK,MP,RWORK(IPSV),WORK(IWU),MP,WORK(IWVT),& 
                   MP,WORK(IWEND),LWORK,RWORK(IPWR),IINFO )
      RK = count( RWORK(IPSV:IPSV+MP-1) > TOL ) 
      time2 = MPI_Wtime()
      IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
          WRITE(*,*) 'ZGESVD costs ', time2 - time1
      ENDIF

!      IF(MYROW.EQ.0 .AND. MYCOL.EQ.0)  write(*,*) 'RK, MP=', RK, MP
!      IF(MYROW.EQ.0 .AND. MYCOL.EQ.0)  write(*,*) 'SV=', RWORK(IPSV:IPSV+MP-1) 
  
      ! Multiply U with Sv, and copy U into WORK(1)
      DO II =1, RK
          IUC=IWU+(II-1)*MP
          CALL ZLASCL('G',0,0,ONE,RWORK(IPSV+II-1),MP,1,WORK(IUC),MP,IINFO)
      END DO
      IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
          WRITE(*,*) 'ZGESVD+Scaling costs ', time2 - time1
      ENDIF
  
      ! Copy the low-rank approximation to the front of WORK
      ! Copy U
      CALL ZLACPY('A',MP,RK,WORK(IWU),MP,WORK(IFTOP),MP)
      IF(2*RK > MP) THEN
!        WRITE(*,*) 'The rank is larger than half of local N, Return'
!        RETURN
      END IF
  
      ! Copy VT
      IWVT1 = IFTOP+MP*RK
      CALL ZLACPY('A',RK,MP,WORK(IWVT),MP,WORK(IWVT1),RK )
      time2 = MPI_Wtime() 
      IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) THEN
          WRITE(*,*) 'Low-rank totally costs ', time2 - time
      ENDIF
!
!     Print the computed low-rank approximation
!      IWEND = IWVT1+MP*RK
!      CALL ZGEMM( 'N','N',MP,MP,RK,ONE,WORK(IFTOP),MP,WORK(IWVT1),RK,&
!                  ZERO,WORK(IWEND),MP )
!      IF(MYROW.EQ.0 .AND. MYCOL.EQ.0) WRITE(*,*) 'CA=',REAL( WORK(IWEND:IWEND+MP*MP-1) )
!	
!   *********************************************************************

!
      SUBROUTINE PSMMA_HANKEL( N,K,ALPHA,A,IA,JA,DESCA,B,LDB, &
...
!  =======
!
!  PSMMA_HANKEL computes the multiplication of a matrix A with a Hankel matrix, 
!  Q = alpha* A * H + beta * Q, where A is N-by-K, and H is K-by-K,
!  H(i,j) = B(i+j-1), which is a Hankel matrix, for i,j=1,...,K.
!  The generators of H are stored in vector B,  and its length is LDB=2K-1.
!  The first row of H from left to right is stored in B(1:K) and the last
!  column from top to bottom is stored in B(K+1:end). B(K) stores the anti-diagonal entry. 
!
...
!  PSMMA_Hankel works for block-cyclic form. The whole procedure is similar to 
!  the Toeplitz matrix. 
!      
...
!
      ...
      IF( .NOT. REDIST ) THEN
            time1 = MPI_Wtime()
            CALL PSHANKEL_COMPUTE( N,K,ALPHA,A,LDA,DESCA,B,LDB,&
                  BETA, Q, LDQ, 0, 0, WORK )
            time2 = MPI_Wtime()
...
!      END IF
      ...
      time1 = MPI_Wtime()
      CALL PSHANKEL_COMPUTE( N,K, ALPHA,WORK(IAT),LDAT,DESCAT,B, LDB,&

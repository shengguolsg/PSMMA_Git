...
!
      IMPLICIT NONE                        
!
!  -- SPMMA Package routine (version 0.1) --
...
!  =======
!
!  PDSMNNBEIG routine is one of the SPMMA package based on block cyclic
!  data distribution on 2-D process configuration. 
!
!  PDSMNNBEIG performs the matrix-matrix operations
!
!     C := alpha*A*B + beta*C,
!
...
!             WRITE(*,*) "We are in pdsmnnbeig, K,KB,NB=", K,KB,NB,IMROW, IMCOL, beta
!
...
!        If empty, use low-rank approximation; 
...
!
         IF ( islowrank ) THEN
            ...
            CALL ConstCauchylowrank( KBP,NQ,B,LDB,WORK(IPB),RIndex,&
                  CIndex,Rk )
...
!            IF(MYROW.EQ.0 .AND. MYCOL.EQ.0 .AND. K1.EQ.1 ) THEN
                  !WRITE(*,*) "B0=", WORK(IPB:IPB+KBP*NQ-1)
!                  write(*,*) "Rindex=", Rindex(1:kbp)
!                  write(*,*) "Cindex=", Cindex(1:nq)
!                  WRITE(*,*) "U1=", B(RIndex(1:KBP),3)
                  !WRITE(*,*) "D1=", B(RIndex(1:KBP),1)
                  !WRITE(*,*) "F1=", B(CIndex(1:NQ),2)
!            END IF
!
             CALL DGEMM( 'N', 'N', MP, NQ, KBP,ALPHA,WORK(IPA), &
                    MAX(1,MP),WORK(IPB),MAX(1,KBP),TBETA, C,LDC )

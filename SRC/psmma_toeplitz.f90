!
      SUBROUTINE PSMMA_TOEPLITZ( N,K,ALPHA,A,IA,JA,DESCA,B,LDB, &
...
!  =======
!
!  PSMMA_TOEPLITZ computes the multiplication of a matrix A with a Toeplitz matrix, 
!  Q = alpha* A * H + beta * Q, where A is N-by-K, and H is K-by-K,
!  H(i,j) = B(K+j-i), which is a Toeplitz matrix. The generators of H are stored in vector B,
!  and its length is LDB=2K-1 or K when H is unsymmetric or symmetric respectively. 
!  The first column of H from bottom to top is stored in B(1:K) and the first row of H 
!  is stored in B(K+1:end) from left to right. B(K) stores the diagonal entry. 
!
...
!  is more efficient. Due to it is not known whether some Toeplitz matrices have
!  low rank structure, and the low-rank approximation part is not implemented yet. 
      
!  PSMMA_Toeplitz works for block-cyclic form. The whole procedure is similar to 
!  the Cauchy-like matrices. 
!      
!  We assume A is N-by-K, and H is K-by-K. Current we assume MB == NB in BCDD. 
! 
! ===========================
! Written by S.G. Li, NUDT, 22th, June. 2022
! =======================================================
!
!     .. Parameters ..
!
...
!
      ...
      IF( .NOT. REDIST ) THEN
            time1 = MPI_Wtime()
            CALL PSTOEPLITZ_COMPUTE( N,K,ALPHA,A,LDA,DESCA,B,LDB,&
                  BETA, Q, LDQ, 0, 0, WORK )
            time2 = MPI_Wtime()

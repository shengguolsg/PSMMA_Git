!
      SUBROUTINE PSHANKEL_COMPUTE( N,K, ALPHA,A,LDA,DESCA,B,LDB,&
...
!     Here B is a Hankel matrix, which is defined by one vector, stored in 
!     the matrix B(LDB). 
...
!           The first K entries stores the first row entries of a Hankel
!           matrix from left to right and B(K) stores the diagonal, and the 
!           last column entries of Hankel is stored in B(K+1:end)
!      
!  LDB    - (input) INTEGER
!           The length of the array , LDB = 2*K-1.
!      
...
!         
         ...
         CALL ConstHankel( KBP,NQ,B,LDB,K,WORK(IPB),RIndex,CIndex )
...
!
    END SUBROUTINE PSHANKEL_COMPUTE
